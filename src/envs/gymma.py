...
class GymmaWrapper(MultiAgentEnv):
    def __init__(
        ...
        **kwargs,
        ...
        sensor_range = kwargs.pop("sensor_range", None)
        if "rware" in key.lower() and sensor_range is not None:
            # Add sensor_range to kwargs only if relevant
            kwargs["sensor_range"] = sensor_range
        self._env = gym.make(f"{key}", **kwargs)
        ## Just double check:
        if "rware" in key.lower():
            print(self._env.unwrapped.sensor_range)
        ...
        self.obs_space = self._env.unwrapped.observation_space
