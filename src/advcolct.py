...
import numpy as np
import random
def advcolct(_run, _config, _log):
    # check args sanity
...
def run_sequential(args, logger):
    ...
    args.noise_params =  {"distrb":"Normal"}
    while current_col_nsteps < args.col_nsteps:
        args.seed = np.random.randint(2**32 - 1, dtype="int64").item() 
        random.seed(args.seed)
        np.random.seed(args.seed)
        th.manual_seed(args.seed)
        runner = r_REGISTRY["advcol"](args=args, logger=logger,infected_idx = list(range(10)),attack_name = "Noise",epsilon= 0.07,alpha = 0,k_iter = 1,noise_params = args.noise_params, align = None)
    ...
    save_data = os.path.join(save_data,args.map_name + "_noise.npz")
